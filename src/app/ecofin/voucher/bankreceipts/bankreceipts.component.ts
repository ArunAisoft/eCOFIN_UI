...
import { BankReceiptsService } from 'src/app/shared/services/voucher/bankreceipts.service';
...
import { BankReceiptSummaryModel, BankReceiptWithDetailsModel, BankReceiptsRequestModel } from 'src/app/shared/models/bankreceipts.models';
...
@Component({
  ...
  styleUrls: ['./bankreceipts.component.css']
})
export class BankReceiptsComponent implements OnInit, OnDestroy {
  ...
  voucherList: BankReceiptSummaryModel[] = [];
  ...
  constructor(private fb: FormBuilder, private commonService: CommonService, private dataService: BankReceiptsService, private alertService: AlertService) { }
  ...
  loadDropdownData(): void {
    ...
    const banks$ = normalizeResponse<any[]>(this.commonService.getAllBanksWithAccountsVouchersBalances(this.userName, 'BNKR'), 'Bank list');

    forkJoin({ years: years$, currencies: currencies$, banks: banks$ })
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: ({ years, currencies, banks }: { years: ApiResponse<any[] | null>; currencies: ApiResponse<any[] | null>; banks: ApiResponse<any[] | null> }) => {
  ...
  onView(): void {
    ...
    this.isLoading = true;
    this.resetAllVoucherFields();
    normalizeResponse<any[]>(this.dataService.getAllBankReceipts(accPeriod), 'Bank Receipts')
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: ApiResponse<any[] | null>) => {
          if (res.status === 200) {
          ...
          this.voucherList = [];
          this.alertService.showCommonError(res.status, res.message, 'Bank Receipts');
        },
        error: (err: any) => {
          this.voucherList = [];
          this.alertService.showCommonError(err?.status || 0, err?.error?.message || err?.message, 'Bank Receipts');
        }
      });
  }

  onVoucherRowClick(item: BankReceiptSummaryModel): void {
  ...
  loadVoucherDetails(onHoldNo: string): void {
    if (!onHoldNo) return;
    this.isLoading = true;
    this.dataService.getBankReceiptWithDetails(onHoldNo)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const payload: BankReceiptWithDetailsModel = (res?.status && res?.data) ? res.data : { header: {}, details: [] };
  ...
  patchVoucher(dto: any): void {
    this.bankBlock.patchValue({
      bank: dto?.bankBlock?.bank ?? null,
      currency: dto?.bankBlock?.currency ?? null,
      bankRate: dto?.bankBlock?.bankRate ?? null
    }, { emitEvent: false });
  ...
  buildPayload(): any | null {
    ...
    const currencyCode = v?.bankBlock?.currency ?? null;
    const bankRate = v?.bankBlock?.bankRate != null ? Number(v.bankBlock.bankRate) : null;
    ...
    if (!details) { this.alertService.warning('Total Credit must be greater than or equal to Total Debit to proceed!'); return null; }
    ...
    return {
      voucherData: {
        ...
        bankCode, bankAccount, bankRate, currencyCode,
        voucherType, voucherSysCategory,
  ...
  onHold(): void {
    ...
    const fy = this.years.find((y: FinancialYearsWithPeriodsModel) => y.financialyear === this.header.get('year')?.value);
    this.months = fy?.periods || [];
    const payload: BankReceiptsRequestModel | null = this.buildPayload();
    if (!payload) return;
    this.isLoading = true;
    this.dataService.saveBankReceiptOnHold(payload).pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
  ...
  onPost(): void {
    ...
    this.isLoading = true;
    this.dataService.saveBankReceiptOnPost(payload).pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: ApiResponse<any>) => {
          if (res?.status === 200 || res?.status === 201) {
