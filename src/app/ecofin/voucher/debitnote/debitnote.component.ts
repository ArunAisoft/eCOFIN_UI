...
import { DebitNotesService } from 'src/app/shared/services/voucher/debitnotes.service';
...
import { DebitNoteSummaryModel, DebitNoteWithDetailsModel, DebitNotesRequestModel } from 'src/app/shared/models/debitnotes.models';
...
export class DebitNoteComponent implements OnInit, OnDestroy {
  ...
  voucherList: DebitNoteSummaryModel[] = [];
  ...
  constructor(
    ...
    private dataService: DebitNotesService,
  ...
  loadDropdownData(): void {
    ...
    const banks$ = normalizeResponse<any[]>(this.commonService.getAllCreditDebitAccountsVouchers(this.userName, 'DEBT'), 'Bank list');
  ...
  onVoucherRowClick(item: DebitNoteSummaryModel): void {
  ...
  loadVoucherDetails(onHoldNo: string): void {
    if (!onHoldNo) return;
    this.isLoading = true;
    this.dataService.getDebitNoteWithDetails(onHoldNo)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const payload: DebitNoteWithDetailsModel = (res?.status && res?.data) ? res.data : { header: {}, details: [] };
          ...
          if (Array.isArray(details) && details.length) {
            dto.lines = details.map((d: any) => {
              ...
              return {
                dc: d?.dbCrFlag ?? 'C', group: d?.accountCode ?? null, account: d?.subAccountCode ?? null,
  ...
  patchVoucher(dto: any): void {
    ...
    dto.lines.forEach((line: any) => {
      const fg = this.fb.group({
        dc: [line.dc ?? 'C'], group: [line.group], account: [line.account], amount: [line.amount],
  ...
  private buildDetails(v: any): any[] | null {
    ...
    if (baseLines.length > 0) {
      ...
      baseLines.push({
        ...
        dbCrFlag: 'D', accountCode: v?.bankBlock?.bankAccount ?? '', subAccountCode: null,
  ...
  buildPayload(): any | null {
    ...
    if (!details) { this.alertService.warning('Total Credit must be less than Total Debit to proceed!'); return null; }
  ...
  onHold(): void {
    ...
    const payload: DebitNotesRequestModel | null = this.buildPayload();
    if (!payload) return;
    this.isLoading = true;
    this.dataService.saveDebitNoteOnHold(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
  ...
  onPost(): void {
    ...
    this.isLoading = true;
    this.dataService.saveDebitNoteOnPost(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
      next: (res: ApiResponse<any>) => {
        if (res?.status === 200 || res?.status === 201) {
