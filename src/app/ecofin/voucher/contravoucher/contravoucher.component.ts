...
import { ContrasService } from 'src/app/shared/services/voucher/contras.service';
...
import { ContraSummaryModel, ContraWithDetailsModel, ContrasRequestModel } from 'src/app/shared/models/contras.models';
...
export class ContraVoucherComponent implements OnInit, OnDestroy {
  ...
  voucherList: ContraSummaryModel[] = [];
  ...
  voucherTypes = [
    { voucherType: 'BCCON', voucherDescription: 'Bank to Cash Contra Voucher' },
    { voucherType: 'BCON', voucherDescription: 'Bank Contra Voucher' },
    { voucherType: 'CBCON', voucherDescription: 'Cash to Bank Contra Voucher' },
    { voucherType: 'CCON', voucherDescription: 'Cash Contra Voucher' }
  ];
  ...
  constructor(
    ...
    private dataService: ContrasService,
  ...
  onView(): void {
    ...
    this.isLoading = true;
    this.resetAllVoucherFields();
    normalizeResponse<any[]>(this.dataService.getAllContras(accPeriod), 'Contra Vouchers')
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: ApiResponse<any[] | null>) => {
          if (res.status === 200) {
            const list = (res.data ?? []) as ContraSummaryModel[];
            if (!list.length) { this.voucherList = []; this.alertService.info('No Contra Vouchers found for the selected Financial Year & Month.'); return; }
            this.voucherList = list;
            this.sortTable('ctrlOnHoldNo');
            return;
          }
          this.voucherList = [];
          this.alertService.showCommonError(res.status, res.message, 'Contra Vouchers');
        },
        error: (err: any) => {
          this.voucherList = [];
          this.alertService.showCommonError(err?.status || 0, err?.error?.message || err?.message, 'Contra Vouchers');
        }
      });
  }

  onVoucherRowClick(item: ContraSummaryModel): void {
  ...
  loadVoucherDetails(onHoldNo: string): void {
    if (!onHoldNo) return;
    this.isLoading = true;
    this.dataService.getContraWithDetails(onHoldNo)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const payload: ContraWithDetailsModel = (res?.status && res?.data) ? res.data : { header: {}, details: [] };
  ...
  onHold(): void {
    ...
    const payload: ContrasRequestModel | null = this.buildPayload();
    if (!payload) return;
    this.isLoading = true;
    this.dataService.saveContraOnHold(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
  ...
  onPost(): void {
    ...
    this.isLoading = true;
    this.dataService.saveContraOnPost(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
      next: (res: ApiResponse<any>) => {
        if (res?.status === 200 || res?.status === 201) {
