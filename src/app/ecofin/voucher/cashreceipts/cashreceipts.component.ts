...
import { CashReceiptsService } from 'src/app/shared/services/voucher/cashreceipts.service';
...
import { CashReceiptSummaryModel, CashReceiptWithDetailsModel, CashReceiptsRequestModel } from 'src/app/shared/models/cashreceipts.models';
import { FinancialYearsWithPeriodsModel, CurrencyModel, BankAccountModel, VoucherTypeModel, GroupAccountModel, GroupSubAccountModel } from 'src/app/shared/models/common.models';
...
export class CashReceiptsComponent implements OnInit, OnDestroy {
  sortColumn: string = '';
  ...
  voucherList: CashReceiptSummaryModel[] = [];
  ...
  constructor(
    ...
    private dataService: CashReceiptsService,
  ...
  createLineGroup(data: any = null): FormGroup {
    ...
    return this.fb.group({
      dc: [data?.dc ?? 'C', Validators.required],   // Cash receipts default to Credit
  ...
  loadDropdownData(): void {
    ...
    const bankAccounts$ = normalizeResponse<any[]>(this.commonService.getAllAccountsVouchersBalances(this.userName, 'CASH', 'CASR'), 'Cash Account list');

    forkJoin({ years: years$, currencies: currencies$, bankAccounts: bankAccounts$ })
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: ({ years, currencies, bankAccounts }) => {
          ...
          this.bankAccounts = (bankAccounts.data ?? []) as BankAccountModel[];
  ...
  onBankAccountChange(): void {
    const bankAccountCode = this.bankBlock.get('bankAccount')?.value as string | null;
  ...
  resetBankBlock(): void {
    this.bankBlock.reset({ bankAccount: null }, { emitEvent: false });
    this.voucherTypes = [];
    this.accountsCache.clear();
  ...
  removeCostCenterRow(i: number): void {
    if (this.costCenterRows.length > 1) { this.costCenterRows.removeAt(i); }
  }
  ...
  cancelCostCenterModal(): void {
    ...
    if (!this.isOnHoldNoGenerated &&
      ...
      this.lines.at(this.currentCostCenterTriggerIndex)) {

      const fg = this.lines.at(this.currentCostCenterTriggerIndex) as FormGroup;
      fg.reset({
        dc: fg.get('dc')?.value ?? 'C',
        group: null, account: null, amount: null,
        receiptDate: null, receiptNo: null, particulars: null,
  ...
  onView(): void {
    ...
    this.isLoading = true;
    this.resetAllVoucherFields();
    normalizeResponse<any[]>(this.dataService.getAllCashReceipts(accPeriod), 'Cash Receipts')
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: ApiResponse<any[] | null>) => {
          if (res.status === 200) {
            const list = (res.data ?? []) as CashReceiptSummaryModel[];
            if (!list.length) {
              this.voucherList = [];
              this.alertService.info('No Cash Receipts found for the selected Financial Year & Month.');
              return;
            }
          ...
          this.voucherList = [];
          this.alertService.showCommonError(res.status, res.message, 'Cash Receipts');
        },
        error: (err: any) => {
          this.voucherList = [];
          this.alertService.showCommonError(err?.status || 0, err?.error?.message || err?.message, 'Cash Receipts');
        }
      });
  }

  onVoucherRowClick(item: CashReceiptSummaryModel): void {
  ...
  loadVoucherDetails(onHoldNo: string): void {
    if (!onHoldNo) return;
    this.isLoading = true;
    this.dataService.getCashReceiptWithDetails(onHoldNo)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const payload: CashReceiptWithDetailsModel = (res?.status && res?.data) ? res.data : { header: {}, details: [] };
          const header = payload.header || {};
          const details = Array.isArray(payload.details) ? payload.details : [];

          const dto: any = {
            bankBlock: { account: (header as any)?.bankAccount ?? null },
            voucherBlock: {
  ...
  applyBankAccountFromPatch(bankAccountCode: string | null): void {
    const selected = this.bankAccounts?.find((b: BankAccountModel) => b.accountCode === bankAccountCode) || null;
    this.balance = selected?.balance ?? 0;
    this.voucherTypes = selected?.voucherTypes ?? [];
  }
  ...
  buildPayload(): any | null {
    ...
    if (!details) {
      this.alertService.warning('Total Credit must be greater than or equal to Total Debit to proceed!');
      return null;
    }
  ...
  onHold(): void {
    ...
    const fy = this.years.find((y: FinancialYearsWithPeriodsModel) => y.financialyear === this.header.get('year')?.value);
    this.months = fy?.periods || [];
    const payload: CashReceiptsRequestModel | null = this.buildPayload();
    if (!payload) return;
    this.isLoading = true;
    this.dataService.saveCashReceiptOnHold(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
  ...
  onPost(): void {
    ...
    this.isLoading = true;
    this.dataService.saveCashReceiptOnPost(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
      next: (res: ApiResponse<any>) => {
        if (res?.status === 200 || res?.status === 201) {
