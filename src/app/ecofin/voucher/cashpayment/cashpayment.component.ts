...
import { CashPaymentsService } from 'src/app/shared/services/voucher/cashpayments.service';
...
import { CashPaymentSummaryModel, CashPaymentWithDetailsModel, CashPaymentsRequestModel } from 'src/app/shared/models/cashpayments.models';
...
export class CashPaymentComponent implements OnInit, OnDestroy {
  ...
  voucherList: CashPaymentSummaryModel[] = [];
  ...
  constructor(
    ...
    private dataService: CashPaymentsService,
  ...
  createLineGroup(data: any = null): FormGroup {
    ...
    return this.fb.group({
      dc: [data?.dc ?? 'D', Validators.required],   // Cash payments default to Debit
  ...
  loadDropdownData(): void {
    ...
    const bankAccounts$ = normalizeResponse<any[]>(this.commonService.getAllAccountsVouchersBalances(this.userName, 'CASH', 'CASP'), 'Cash Account list');
  ...
  onView(): void {
    ...
    this.isLoading = true;
    this.resetAllVoucherFields();
    normalizeResponse<any[]>(this.dataService.getAllCashPayments(accPeriod), 'Cash Payments')
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: ApiResponse<any[] | null>) => {
          if (res.status === 200) {
            const list = (res.data ?? []) as CashPaymentSummaryModel[];
            if (!list.length) {
              this.voucherList = [];
              this.alertService.info('No Cash Payments found for the selected Financial Year & Month.');
              return;
            }
          ...
          this.voucherList = [];
          this.alertService.showCommonError(res.status, res.message, 'Cash Payments');
        },
        error: (err: any) => {
          this.voucherList = [];
          this.alertService.showCommonError(err?.status || 0, err?.error?.message || err?.message, 'Cash Payments');
        }
      });
  }

  onVoucherRowClick(item: CashPaymentSummaryModel): void {
  ...
  loadVoucherDetails(onHoldNo: string): void {
    if (!onHoldNo) return;
    this.isLoading = true;
    this.dataService.getCashPaymentWithDetails(onHoldNo)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const payload: CashPaymentWithDetailsModel = (res?.status && res?.data) ? res.data : { header: {}, details: [] };
  ...
  buildPayload(): any | null {
    ...
    if (!details) {
      this.alertService.warning('Total Debit must be greater than or equal to Total Credit to proceed!');
      return null;
    }
  ...
  onHold(): void {
    ...
    const fy = this.years.find((y: FinancialYearsWithPeriodsModel) => y.financialyear === this.header.get('year')?.value);
    this.months = fy?.periods || [];
    const payload: CashPaymentsRequestModel | null = this.buildPayload();
    if (!payload) return;
    this.isLoading = true;
    this.dataService.saveCashPaymentOnHold(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
  ...
  onPost(): void {
    ...
    this.isLoading = true;
    this.dataService.saveCashPaymentOnPost(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
      next: (res: ApiResponse<any>) => {
        if (res?.status === 200 || res?.status === 201) {
