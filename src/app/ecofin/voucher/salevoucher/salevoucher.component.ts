...
import { SalesService } from 'src/app/shared/services/voucher/sales.service';
...
import { SaleSummaryModel, SaleWithDetailsModel, SalesRequestModel, SaleImportModel } from 'src/app/shared/models/sales.models';
...
@Component({
  selector: 'app-salevoucher',
  templateUrl: './salevoucher.component.html',
  styleUrls: ['./salevoucher.component.css']
})
export class SaleVoucherComponent implements OnInit, OnDestroy {
  ...
  activeSortType: 'voucher' | 'domestic' | 'export' = 'voucher';
  form!: FormGroup;
  ...
  currentImportType = '0';
  domesticList: SaleImportModel[] = [];
  exportList: SaleImportModel[] = [];
  searchText = '';
  maxSelection = 50;
  ...
  constructor(private fb: FormBuilder, private commonService: CommonService, private dataService: SalesService, private alertService: AlertService) { }
  ...
  createBillGroup(data: any = null): FormGroup {
    return this.fb.group({
      ...
      billDate: [data?.billDate ?? null, [Validators.required]]
    });
  }
  ...
  loadDropdownData(): void {
    ...
    const banks$ = normalizeResponse<any[]>(this.commonService.getAllDebitAccountsVouchers(this.userName, 'SALV'), 'Group Account');
    const voucherTypes$ = normalizeResponse<any[]>(this.commonService.getAllDebitCreditVoucherTypes(this.userName, 'SALV'), 'Voucher Types');

    forkJoin({ years: years$, banks: banks$, voucherTypes: voucherTypes$ })
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: ({ years, banks, voucherTypes }) => {
        ...
        error: () => {
          ...
          this.banks = []; this.bankAccounts = []; this.voucherTypes = [];
        }
      });
  }
  ...
  onView(): void {
    ...
    this.selectVouchersAll = false;
    this.resetAllVoucherFields();
    normalizeResponse<any[]>(this.dataService.getAllSales(accPeriod), 'Sales')
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: ApiResponse<any[] | null>) => {
          if (res.status === 200) {
          ...
          this.voucherList = [];
          this.alertService.showCommonError(res.status, res.message, 'Sales');
        },
        error: (err: any) => { this.voucherList = []; this.alertService.showCommonError(err?.status || 0, err?.error?.message || err?.message, 'Sales'); }
      });
  }

  onVoucherRowClick(item: SaleSummaryModel): void {
  ...
  loadVoucherDetails(onHoldNo: string): void {
    if (!onHoldNo) return;
    this.isLoading = true;
    this.dataService.getSaleWithDetails(onHoldNo)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const payload: SaleWithDetailsModel = (res?.status && res?.data) ? res.data : { header: {}, details: [], billDetails: {} };
          ...
          const dto: any = {
            ...
            billDetails: [{ billNo: (billDetails as any)?.billNo ?? null, billAmount: (billDetails as any)?.billAmount ?? null, billDate: (billDetails as any)?.billDate ?? null }],
            lines: [] as any[],
  ...
  patchVoucher(dto: any): void {
    ...
    try {
      ...
      if (billData) { billData.billDate = this.toDateInputValue(billData?.billDate); }
  ...
  private buildDetails(v: any): any[] | null {
    ...
    const baseLines = (v?.lines ?? []).filter((l: any) => l != null).map((l: any) => {
      ...
      return {
        ...
        dbCrFlag: l?.dc ?? 'C', accountCode, subAccountCode: subAccountCode || null,
    ...
    if (billAmount !== creditTotal) return null;
    ...
    if (baseLines.length > 0) {
      ...
      baseLines.push({
        ...
        dbCrFlag: 'D',
        accountCode: v?.bankBlock?.bank ?? '',
        subAccountCode: v?.bankBlock?.bankAccount ?? '',
        drCrAmount: parseFloat((creditTotal - debitTotal).toFixed(2)),
  ...
  buildPayload(): any | null {
    ...
    if (!details) { this.alertService.warning('Total Bill Amount must equal Total Credit OR Total Credit must be greater than Total Debit to proceed!'); return null; }

    const bill = (v?.billDetails && v.billDetails.length) ? v.billDetails[0] : null;
    return {
      voucherData: { ctrlOnHoldNo: this.onHoldNo ?? '', voucherNumber: this.voucherNo ?? '', voucherDate: this.toIsoDate(v?.voucherBlock?.voucherDate), bankCode: v?.bankBlock?.bank ?? '', bankAccount: v?.bankBlock?.bankAccount ?? '', voucherType: v?.voucherBlock?.voucherType ?? '', voucherSysCategory: v?.voucherBlock?.voucherSysCategory ?? 'REGUL', voucherNarration: v?.voucherBlock?.narration ?? '', billNo: bill?.billNo ?? null, billAmount: bill?.billAmount ?? null, billDate: this.toIsoDate(bill?.billDate), createdOn: null, updatedOn: null, balance: null },
      details,
      billDetails: { billNo: bill?.billNo ?? '', billDate: this.toIsoDate(bill?.billDate), billAmount: bill?.billAmount ?? null },
  ...
  onHold(): void {
    ...
    const payload: SalesRequestModel | null = this.buildPayload();
    if (!payload) return;
    ...
    this.commonService.checkBillExists(payload.voucherData.bankCode, payload.voucherData.bankAccount, payload.billDetails.billNo, billDate, this.onHoldNo ?? null).subscribe(res => {
      ...
      this.dataService.saveSaleOnHold(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
  ...
  onPost(): void {
    ...
    this.commonService.checkBillExists(payload.voucherData.bankCode, payload.voucherData.bankAccount, payload.billDetails.billNo, billDate, this.onHoldNo ?? null).subscribe(res => {
      if (res?.data) { this.alertService.error('Bill already exists for selected Bank and Date.'); this.isLoading = false; return; }
      this.dataService.saveSaleOnPost(payload).pipe(finalize(() => (this.isLoading = false))).subscribe({
        next: (res: ApiResponse<any>) => {
          if (res?.status === 200 || res?.status === 201) {
  ...
  sortTable(column: string, type: 'voucher' | 'domestic' | 'export' = 'voucher', isDefault: boolean = false): void {
    ...
    if (type === 'domestic') { this.domesticList = [...this.domesticList].sort(sortFn); }
    else if (type === 'export') { this.exportList = [...this.exportList].sort(sortFn); }
    else { this.voucherList = [...this.voucherList].sort(sortFn); }
  }

  clearImportLists = (): void => { this.domesticList = []; this.exportList = []; };
  ...
  onImportTypeChange = (): void => {
    ...
    if (this.currentImportType === '1') this.loadDomesticData(fromDate, toDate);
    else if (this.currentImportType === '2') this.loadExportData(fromDate, toDate);
  ...
  loadDomesticData = (fromDate: string, toDate: string): void => {
    this.isLoading = true;
    this.domesticList = [];
    this.selectAll = false;
    this.dataService.getSaleDomesticData(fromDate, toDate)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const data: SaleImportModel[] = (res?.status && res?.data) ? res.data : (Array.isArray(res) ? res : []);
          this.domesticList = data.map((item: SaleImportModel) => ({ ...item, selected: false }));
          this.sortTable('invoiceNo', 'domestic', true);
        },
        error: (err: any) => { console.error(err); this.alertService.error('Error loading Domestic data'); }
      });
  };

  loadExportData = (fromDate: string, toDate: string): void => {
    this.isLoading = true;
    this.exportList = [];
    this.selectAll = false;
    this.dataService.getSaleExportData(fromDate, toDate)
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (res: any) => {
          const data: SaleImportModel[] = (res?.status && res?.data) ? res.data : (Array.isArray(res) ? res : []);
          this.exportList = data.map((item: SaleImportModel) => ({ ...item, selected: false }));
          this.sortTable('invoiceNo', 'export', true);
        },
        error: (err: any) => { console.error(err); this.alertService.error('Error loading Export data'); }
      });
  };
  ...
  importSelectedDomesticToBills = (): void => {
    if (this.header.invalid) { this.header.markAllAsTouched(); return; }
    const v = this.form.value;
    ...
    const selected = this.domesticList.filter(g => g.selected);
    ...
    this.dataService.OnHoldDomesticSales(payload).pipe(finalize(() => this.isLoading = false)).subscribe({
      next: () => {
        ...
        if (this.voucherList.length > 0) this.onView();
        this.alertService.success(`${selected.length} Domestic(s) imported into Bills.`);
      },
      error: () => { this.alertService.error('Something went wrong'); }
    });
  };

  importSelectedExportToBills = (): void => {
    if (this.header.invalid) { this.header.markAllAsTouched(); return; }
    const v = this.form.value;
    if (!v?.voucherBlock?.voucherType) { this.alertService.warning('Voucher Type is required'); return; }
    // if (!v?.voucherBlock?.voucherDate) { this.alertService.warning('Voucher Date is required'); return; }
    const selected = this.exportList.filter(j => j.selected);
    ...
    this.dataService.OnHoldExportSales(payload).pipe(finalize(() => this.isLoading = false)).subscribe({
      next: () => {
        ...
        if (this.voucherList.length > 0) this.onView();
        this.alertService.success(`${selected.length} Export(s) imported into Bills.`);
      },
      error: () => { this.alertService.error('Something went wrong'); }
    });
  };
  ...
  get filteredVouchersList(): SaleSummaryModel[] {
