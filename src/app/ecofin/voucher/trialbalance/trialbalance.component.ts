...
export class TrialBalanceComponent implements OnInit, OnDestroy {
  ...
  selectedAccountDescription: string | null = null;   // NEW: description for the selected account
  selectedSubAccountCode: string | null = null;
  selectedSubAccountDescription: string | null = null; // NEW: description for the selected sub-account
  ...
  // ── Label helpers ────────────────────────────────────────────────────────
  // Returns "CODE - Description" or just "CODE" when description is absent.
  private accountLabel(): string {
    const code = this.selectedAccountCode || '';
    const desc = this.selectedAccountDescription || '';
    return desc ? `${code} - ${desc}` : code;
  }

  private subAccountLabel(): string {
    const code = this.selectedSubAccountCode || '';
    const desc = this.selectedSubAccountDescription || '';
    return desc ? `${code} - ${desc}` : code;
  }

  // ── Period label ─────────────────────────────────────────────────────────
  // Returns "DD-MM-YYYY — DD-MM-YYYY" based on the selected accPeriod.
  // Falls back to the raw accperiod string if dates cannot be resolved.
  private getPeriodLabel(): string {
    ...
    if (!fromDateIso || !toDateIso) return accPeriod;
    const fmt = (iso: string) => {
      const [y, m, d] = iso.split('-');
      return `${d}-${m}-${y}`;
    };
    return `${fmt(fromDateIso)} — ${fmt(toDateIso)}`;
  }
  ...
  onTrialBalanceAccountClick(row: TrialBalanceRow): void {
    ...
    this.selectedAccountDescription = row.description || null; // capture description
    this.selectedSubAccountCode = null;
    this.selectedSubAccountDescription = null;
  ...
  onSubledgerScheduleClick(row: SubledgerScheduleRow): void {
    ...
    this.selectedSubAccountDescription = row.subCodeDescription || null; // capture sub description
    this.selectedAccountCode = row.accountCode;
    this.selectedAccountDescription = row.description || null;           // refresh account description
    this.billsPaymentsRows = [];
    this.filteredBillsPayments = [];
  ...
  breadcrumbClick(bc: any): void {
    ...
    if (bc.tab === 'trialBalance') {
      ...
      const { fromDateIso, toDateIso } = this.getFromToIso(accPeriod);
      if (accPeriod && fromDateIso && toDateIso && bc.accountCode)
        this.glDetailsTrigger$.next({ accPeriod, accCode: bc.accountCode, from: fromDateIso, to: toDateIso });
  ...
  downloadVoucherPdf(): void {
    ...
    (doc as any).autoTable({
      ...
      body,
      theme: 'grid',
      styles: { fontSize: 7.5, cellPadding: { top: 1.5, bottom: 1.5, left: 2, right: 2 }, overflow: 'linebreak', textColor: [30, 30, 30], lineColor: [221, 221, 221], lineWidth: 0.2 },
      headStyles: { fillColor: this.G1, textColor: this.WHT, fontStyle: 'bold', halign: 'left' },
      columnStyles: {
        0: { cellWidth: 100 }, 1: { cellWidth: 35 },
      ...
      didParseCell: (data: any) => {
        ...
        if (data.row.index === body.length - 1) {
          data.cell.styles.fillColor = this.G1; data.cell.styles.textColor = this.WHT; data.cell.styles.fontStyle = 'bold';
  ...
  downloadCostProductPdf(): void {
    ...
    for (const grp of this.costProductGrouped) {
      body.push([{ content: `${grp.accountCode} : ${grp.description}`, colSpan: 2, styles: { fontStyle: 'bold' as const, textColor: [45, 106, 45] } }]);
      rowMeta.push('group');
    ...
    (doc as any).autoTable({
      ...
      body, theme: 'grid',
      styles: { fontSize: 8, cellPadding: { top: 2, bottom: 2, left: 3, right: 3 }, overflow: 'linebreak', textColor: [30, 30, 30], lineColor: [221, 221, 221], lineWidth: 0.2 },
      headStyles: { fillColor: this.G1, textColor: this.WHT, fontStyle: 'bold', halign: 'left' },
      columnStyles: { 0: { cellWidth: 130 }, 1: { cellWidth: 50, halign: 'right' as const } },
      didParseCell: (data: any) => {
        if (data.section !== 'body') return;
        if (rowMeta[data.row.index] === 'acctotal') {
          data.cell.styles.fillColor = this.MID; data.cell.styles.fontStyle = 'bold';
  ...
  private applyExcelStyles(ws: any, aoaRows: any[][], rowTypes: string[], numCols: number[], totalCols: number): void {
    ...
    aoaRows.forEach((row, ri) => {
      ...
      for (let c = 0; c < totalCols; c++) {
        ...
        switch (type) {
          case 'company':
            ws[addr].s = { font: { bold: true, sz: 12, color: { rgb: '4EAA39' } }, alignment: { horizontal: 'center', vertical: 'center' } };
            break;
          case 'title':
            ws[addr].s = { font: { bold: true, sz: 11 }, alignment: { horizontal: 'center', vertical: 'center' } };
            break;
          case 'header':
  ...
  downloadVoucherExcel(): void {
    ...
    aoaRows.push([`Voucher Details — ${this.voucherPopupTitle} dated ${this.voucherPopupDateRaw}`, '', '', '']); rowTypes.push('title');
    aoaRows.push(Array(4).fill('')); rowTypes.push('blank');
    aoaRows.push(['Account Description / Line Particulars', 'Inst. No / Date', 'Debit', 'Credit']); rowTypes.push('header');

    for (const e of this.voucherEntries) {
      aoaRows.push([
        [e.accountCode, e.description, e.lineDetails, e.subAccountCode ? `${e.subAccountCode} : ${e.subCodeDescription}` : ''].filter(Boolean).join(' | '),
    ...
    aoaRows.push(['Voucher Totals', '', this.voucherTotalDebit, this.voucherTotalCredit]); rowTypes.push('gtotal');
  ...
  exportBillsAdjustedToExcel(): void {
    ...
    aoaRows.push([`Bills/Payments Adjusted — ${this.voucherPopupTitle}`, '', '', '', '', '', '']); rowTypes.push('title');
    aoaRows.push(Array(7).fill('')); rowTypes.push('blank');
    aoaRows.push(['Voucher Number', 'Voucher Date', 'Reference Number', 'Reference Date', 'Bill Number', 'Bill Date', 'Bill Adjusted Amount']); rowTypes.push('header');
    ...
    const grandTotal = this.billsAdjustedRows.reduce((s, r) => s + (Number(r.billAdjustedAmount) || 0), 0);
    aoaRows.push(['', '', '', '', '', 'Grand Total', grandTotal]); rowTypes.push('gtotal');
  ...
  exportCostProductToExcel(): void {
    ...
    aoaRows.push([`Cost / Product Entries — ${this.voucherPopupTitle}`, '', '', '', '']); rowTypes.push('title');
    aoaRows.push(Array(5).fill('')); rowTypes.push('blank');
    aoaRows.push(['Account Code', 'Account Description', 'Cost Centre Code', 'Cost Centre Description', 'Amount']); rowTypes.push('header');
    ...
    for (const grp of this.costProductGrouped) {
      ...
      aoaRows.push(['', '', '', 'Account Total', grp.total]); rowTypes.push('vtotal');
    }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // ACTIVE-TAB PDF / EXCEL DISPATCHERS
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  downloadTrialBalancePdf(): void {
    ...
    const pageW = doc.internal.pageSize.getWidth();
    const today = new Date().toLocaleDateString('en-GB');
    const period = this.getPeriodLabel(); // ← date range
    ...
    const body: any[] = this.filteredTrialBalance.map(r => [
      r.accountCode, r.description, this.fmt(r.debit || 0), this.fmt(r.credit || 0)
    ]);
  ...
  downloadGlDetailsPdf(): void {
    ...
    const pageW = doc.internal.pageSize.getWidth();
    const today = new Date().toLocaleDateString('en-GB');
    const accLabel = this.accountLabel();  // ← "CODE - Description"
    const period = this.getPeriodLabel();  // ← date range
    ...
    doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(90, 136, 96);
    doc.text(`Account: ${accLabel}    |    Period: ${period}    |    Printed on: ${today}`, pageW / 2, 21, { align: 'center' });
    doc.setTextColor(0, 0, 0);

    const body: any[] = [];
    if (this.glOpeningBalance !== 0) {
      body.push(['', '', 'Opening Balance', this.fmt(this.glOpeningDebit), this.fmt(this.glOpeningCredit), '']);
    }
    ...
    body.push(['', '', 'Closing Balance', this.fmt(this.glClosingDebit), this.fmt(this.glClosingCredit), '']);
    ...
    doc.save(`GLDetails_${this.selectedAccountCode}.pdf`);
  }
  ...
  downloadGlDetailsExcel(): void {
    ...
    aoa.push(['OTTO BILZ (INDIA) PVT. LTD.', '', '', '', '', '']); types.push('company');
    aoa.push([`GL Account Details — ${accLabel}    |    Period: ${period}`, '', '', '', '', '']); types.push('title');
    aoa.push(Array(6).fill('')); types.push('blank');
    aoa.push(['Voucher No', 'Date', 'Type', 'Debit', 'Credit', 'Details']); types.push('header');
    if (this.glOpeningBalance !== 0) {
      aoa.push(['', '', 'Opening Balance', this.glOpeningDebit || null, this.glOpeningCredit || null, '']);
      types.push('gtotal');
    }
    for (const r of this.filteredGlDetails) {
      aoa.push([
        r.voucherNumber,
        r.voucherDate ? new Date(r.voucherDate).toLocaleDateString('en-GB') : '',
        r.voucherType, r.debit || null, r.credit || null, r.lineDetails || ''
      ]);
      types.push('data');
    }
    aoa.push(['', '', 'Total', this.totalglDebit, this.totalglCredit, '']); types.push('gtotal');
    aoa.push(['', '', 'Closing Balance', this.glClosingDebit || null, this.glClosingCredit || null, '']); types.push('gtotal');
    ...
    XLSX.writeFile(wb, `GLDetails_${this.selectedAccountCode}.xlsx`);
  }
  ...
  downloadSubledgerSchedulePdf(): void {
    ...
    const body: any[] = this.filteredSubledgerSchedule.map(r => [
      ...
      this.fmt(Number(r.debit) || 0), this.fmt(Number(r.credit) || 0)
    ]);
    ...
    (doc as any).autoTable({
      ...
      headStyles: { fillColor: this.G1, textColor: this.WHT, fontStyle: 'bold' },
      columnStyles: {
        0: { cellWidth: 30 }, 1: { cellWidth: 65 }, 2: { cellWidth: 30 }, 3: { cellWidth: 85 },
    ...
    doc.save(`SubledgerSchedule_${this.selectedAccountCode}.pdf`);
  }
  ...
  downloadSubledgerScheduleExcel(): void {
    ...
    aoa.push(['OTTO BILZ (INDIA) PVT. LTD.', '', '', '', '', '']); types.push('company');
    aoa.push([`Sub Ledger Schedule — ${accLabel}    |    Period: ${period}`, '', '', '', '', '']); types.push('title');
    aoa.push(Array(6).fill('')); types.push('blank');
    ...
    XLSX.writeFile(wb, `SubledgerSchedule_${this.selectedAccountCode}.xlsx`);
  }
  ...
  downloadSubledgerAccountPdf(): void {
    ...
    const today = new Date().toLocaleDateString('en-GB');
    const accLabel = this.accountLabel();    // ← "CODE - Description"
    const subLabel = this.subAccountLabel(); // ← "CODE - Description"
    const period = this.getPeriodLabel();    // ← date range
    ...
    doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(90, 136, 96);
    doc.text(`Account: ${accLabel}    |    Sub Account: ${subLabel}    |    Period: ${period}    |    Printed on: ${today}`, pageW / 2, 21, { align: 'center' });
    doc.setTextColor(0, 0, 0);

    const body: any[] = [];
    if (this.subledgerOpeningBalance !== 0) {
      body.push(['', '', '', 'Opening Balance', this.fmt(this.subledgerOpeningDebit), this.fmt(this.subledgerOpeningCredit)]);
    }
    ...
    body.push(['', '', '', 'Closing Balance', this.fmt(this.subledgerClosingDebit), this.fmt(this.subledgerClosingCredit)]);

    (doc as any).autoTable({
      startY: 25, margin: { left: 10, right: 10 },
      head: [['Voucher No', 'Voucher Date', 'Invoice No', 'Invoice Date', 'Debit', 'Credit']],
      body, theme: 'grid',
      styles: { fontSize: 7.5, cellPadding: { top: 1.5, bottom: 1.5, left: 2, right: 2 }, overflow: 'linebreak', lineColor: [221, 221, 221], lineWidth: 0.2 },
      headStyles: { fillColor: this.G1, textColor: this.WHT, fontStyle: 'bold' },
      columnStyles: {
        0: { cellWidth: 35 }, 1: { cellWidth: 35 }, 2: { cellWidth: 50 }, 3: { cellWidth: 50 },
        4: { cellWidth: 35, halign: 'right' as const },
    ...
    doc.save(`SubledgerAccount_${this.selectedSubAccountCode}.pdf`);
  }
  ...
  downloadSubledgerAccountExcel(): void {
    ...
    aoa.push(['OTTO BILZ (INDIA) PVT. LTD.', '', '', '', '', '']); types.push('company');
    aoa.push([`Subledger Account Details — ${accLabel} / ${subLabel}    |    Period: ${period}`, '', '', '', '', '']); types.push('title');
    aoa.push(Array(6).fill('')); types.push('blank');
    aoa.push(['Voucher No', 'Voucher Date', 'Invoice No', 'Invoice Date', 'Debit', 'Credit']); types.push('header');
    if (this.subledgerOpeningBalance !== 0) {
      aoa.push(['', '', '', 'Opening Balance', this.subledgerOpeningDebit || null, this.subledgerOpeningCredit || null]);
      types.push('gtotal');
    }
    for (const r of this.filteredSubledgerAccount) {
      aoa.push([
        r.voucherNumber,
        r.voucherDate ? new Date(r.voucherDate).toLocaleDateString('en-GB') : '',
        r.invoiceNo, r.invoiceDate ? new Date(r.invoiceDate).toLocaleDateString('en-GB') : '',
        r.debit || null, r.credit || null
      ]);
      types.push('data');
    }
    aoa.push(['', '', '', 'Total', this.totalslaDebit, this.totalslaCredit]); types.push('gtotal');
    aoa.push(['', '', '', 'Closing Balance', this.subledgerClosingDebit || null, this.subledgerClosingCredit || null]); types.push('gtotal');

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = [{ wch: 18 }, { wch: 18 }, { wch: 25 }, { wch: 25 }, { wch: 16 }, { wch: 16 }];
    ws['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 5 } }, { s: { r: 1, c: 0 }, e: { r: 1, c: 5 } }];
    this.applyExcelStyles(ws, aoa, types, [4, 5], 6);
    XLSX.utils.book_append_sheet(wb, ws, 'Subledger Account');
    XLSX.writeFile(wb, `SubledgerAccount_${this.selectedSubAccountCode}.xlsx`);
  }
  ...
  downloadBillsPaymentsPdf(): void {
    ...
    doc.save(`BillsPayments_${this.selectedSubAccountCode || this.selectedAccountCode}.pdf`);
  }
  ...
  downloadBillsPaymentsExcel(): void {
    ...
    aoa.push(['OTTO BILZ (INDIA) PVT. LTD.', '', '', '', '', '', '', '']); types.push('company');
    aoa.push([`Open Bills and Payments — ${accLabel} / ${subLabel}    |    Period: ${period}`, '', '', '', '', '', '', '']); types.push('title');
    ...
    XLSX.writeFile(wb, `BillsPayments_${this.selectedSubAccountCode || this.selectedAccountCode}.xlsx`);
  }
  ...
  private extractOpeningBalance(rows: any[]): number {
    const openingRow = rows.find(r => (r.sequenceNo || r.SequenceNo) === '2');
    return openingRow ? Number(openingRow.openingBalance || openingRow.OpeningBalance || 0) : 0;
  }

  private extractGlOpeningBalance(rows: any[]): number {
    const openingRow = rows.find(r => {
      const seq = r.sequenceNo || r.SequenceNo || '';
      return seq === '2' || seq === '1';
    });
    if (!openingRow) return 0;
    const dr = Number(openingRow.debit || openingRow.Debit || 0);
    const cr = Number(openingRow.credit || openingRow.Credit || 0);
    return dr - cr;
  }
  ...
  private setupStreams(): void {
    ...
    this.glDetailsTrigger$.pipe(
      ...
      next: res => {
        ...
        const seq = (r: any) => r.sequenceNo || r.SequenceNo || '';
        this.glDetails = allRows.filter(r => seq(r) !== '1' && seq(r) !== '2');
