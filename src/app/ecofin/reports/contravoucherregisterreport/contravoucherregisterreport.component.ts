...
import { ContraReportService } from 'src/app/shared/services/reports/contravoucherregisterreport.service';
...
export interface ContraVoucherGroup {
  voucherDate: string;
...
@Component({
  selector: 'app-contravoucherregisterreport',
  templateUrl: './contravoucherregisterreport.component.html',
  styleUrls: ['./contravoucherregisterreport.component.css']
})
export class ContravoucherregisterreportComponent implements OnInit {

  isLoading = false;
  filterForm!: FormGroup;
  voucherGroups: ContraVoucherGroup[] = [];
  accPeriodLabel = '';
  ...
  constructor(
    ...
    private svc: ContraReportService,
  ...
  onRetrieve(): void {
    ...
    this.svc.getContraReport({ accPeriod: this.accPeriodLabel })
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (resp: any) => {
        ...
        error: () => this.alertService.error('Failed to retrieve Contra report.')
      });
  }
  ...
  private groupByVoucher(rows: any[]): ContraVoucherGroup[] {
    const map = new Map<string, ContraVoucherGroup>();
    for (const r of rows) {
  ...
  trackByVoucherGroup = (_: number, g: ContraVoucherGroup) => g.voucherNumber;
  ...
  downloadPdf(): void {
    ...
    const fname = `ContraRegister_${this.accPeriodLabel}.pdf`;

    doc.setFont('helvetica', 'bold');
    doc.setFontSize(13);
    doc.setTextColor(0, 0, 0);
    doc.text(
      `CONTRA REGISTER FOR THE MONTH OF ${this.accPeriodLabel.toUpperCase()}`,
    ...
    for (const grp of this.voucherGroups) {
      grp.rows.forEach((r, i) => {
        ...
        body.push([
          ...
          i === 0 ? grp.onHoldNo : '',
          r.accountCode ?? '',
          [r.accountDesc, r.lineParticulars].filter(Boolean).join('\n'),
          [r.instrumentNo, r.instrumentDate].filter(Boolean).join('\n'),
    ...
    (doc as any).autoTable({
      ...
      head: [['Voucher Date', 'Voucher No', 'Onhold No', 'A/C Code', 'Description / Line Particulars', 'Chq.No / Date', 'Ref.No / Date', 'Debit', 'Credit', 'Narration']],
      body,
      theme: 'grid',
      styles: {
        fontSize: 7,
  ...
  downloadExcel(): void {
    ...
    aoaRows.push(['Voucher Date', 'Voucher No', 'Onhold No', 'A/C Code', 'Account Description', 'Line Particulars', 'Chq.No', 'Chq.Date', 'Ref.No', 'Ref.Date', 'Debit (Rs.)', 'Credit (Rs.)', 'Narration']);
    ...
    ws['!cols'] = [
      { wch: 13 }, { wch: 16 }, { wch: 16 }, { wch: 10 },
      { wch: 28 }, { wch: 28 }, { wch: 12 }, { wch: 12 },
      { wch: 12 }, { wch: 12 }, { wch: 16 }, { wch: 16 }, { wch: 22 }
    ];
    ...
    XLSX.writeFile(wb, `ContraRegister_${this.accPeriodLabel}.xlsx`);
  }
