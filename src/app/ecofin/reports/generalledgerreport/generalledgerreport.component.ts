...
import { GeneralLedgerReportService } from 'src/app/shared/services/reports/generalledgerreport.service';
...
export class GeneralLedgerReportComponent implements OnInit {
  ...
  constructor(
    ...
    private svc: GeneralLedgerReportService,
  ...
  onRetrieve(): void {
    ...
    this.svc.getGeneralLedger(v.fromDate, v.toDate)
      ...
      .subscribe({
        ...
        error: () => this.alertService.error('Failed to retrieve General Ledger.')
      });
  }
  ...
  get totalAmount(): number {
    return this.reportRows.reduce((s, r) => s + (Number(r.voucherAmount) || 0), 0);
  }
  ...
  downloadPdf(): void {
    ...
    doc.text('General Ledger', 210, 12, { align: 'center' });
    doc.setFontSize(8); doc.setTextColor(80, 80, 80);
    doc.text(`Period From: ${v.fromDate}  To: ${v.toDate}`, 210, 18, { align: 'center' });

    const cols = ['Account', 'Description', 'Vchr No', 'Date', 'Particulars', 'Ref No', 'Inst No', 'Amount', 'Closing Bal'];
    const body = this.reportRows.map(r => [
      r.accountCode, r.accountDescription, r.vchrNumber, r.vchrDate,
      r.lineDetails, r.vchrRefNumber, r.instrumentNo, fmtN(r.voucherAmount), fmtN(r.closingBalance)
    ]);
    ...
    doc.save(`General_Ledger_${v.fromDate}_To_${v.toDate}.pdf`);
  }
  ...
  downloadExcel(): void {
    ...
    aoa.push([`General Ledger — Period From: ${v.fromDate}  To: ${v.toDate}`]); types.push('title');
    ...
    for (const r of this.reportRows) {
      aoa.push([r.accountCode, r.accountDescription, r.vchrNumber, r.vchrDate,
                r.lineDetails, r.vchrRefNumber, r.instrumentNo, fmtN(r.voucherAmount), fmtN(r.closingBalance)]);
      types.push('data');
    }
    ...
    XLSX.writeFile(wb, `General_Ledger_${v.fromDate}_To_${v.toDate}.xlsx`);
  }
}
