...
import { DebitNoteReportService } from 'src/app/shared/services/reports/debitnoteregisterreport.service';
...
export interface DebitNoteVoucherGroup {
  voucherDate      : string;   // dn.VchrDate
  voucherNumber    : string;   // dn.VchrNumber
  onHoldNo         : string;   // dn.CtrlOnholdno
  narration        : string;   // dn.VchrNarration
  hdrAccountCode   : string;   // dn.Accountcode
  hdrSubAccountCode: string;   // dn.Subaccountcode
...
@Component({
  selector   : 'app-debitnoteregisterreport',
  templateUrl: './debitnoteregisterreport.component.html',
  styleUrls  : ['./debitnoteregisterreport.component.css']
})
export class DebitnoteregisterreportComponent implements OnInit {

  isLoading      = false;
  filterForm!    : FormGroup;
  voucherGroups  : DebitNoteVoucherGroup[] = [];
  accPeriodLabel = '';
  ...
  constructor(
    ...
    private svc          : DebitNoteReportService,
  ...
  onRetrieve(): void {
    ...
    this.svc.getDebitNoteReport({ accPeriod: this.accPeriodLabel })
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (resp: any) => {
        ...
        error: () => this.alertService.error('Failed to retrieve Debit Note report.')
      });
  }
  ...
  //   costTypeDescription   = costType.Parameterdescription
  //   expenseTypeDescription= expenseType.Parameterdescription

  private groupByVoucher(rows: any[]): DebitNoteVoucherGroup[] {
    const map = new Map<string, DebitNoteVoucherGroup>();
    for (const r of rows) {
  ...
  trackByVoucherGroup = (_: number, g: DebitNoteVoucherGroup) => g.voucherNumber;
  ...
  // =========================================================================
  // PDF DOWNLOAD
  // Matches Debitnote.pdf layout exactly:
  //   Onhold No | Date | Vchr No | Vendor Code & Name | Narration
  ...
  downloadPdf(): void {
    ...
    const fname = `DebitNoteRegister_${this.accPeriodLabel}.pdf`;
    ...
    doc.text(
      `DEBIT NOTE REGISTER FOR THE MONTH OF ${this.accPeriodLabel.toUpperCase()}`,
    ...
    for (const grp of this.voucherGroups) {
      grp.rows.forEach((r, i) => {
        ...
        // Vendor cell: hdrAccountCode + hdrSubAccountCode + hdrSubAccountDesc (once per group)
        ...
        const costInfo = [
          r.costCentreCode,
          r.expenseTypeDescription,
          r.costTypeDescription
        ].filter(Boolean).join(' / ');
        ...
        body.push([
          i === 0 ? grp.onHoldNo      : '',   // col 0
          i === 0 ? grp.voucherDate   : '',   // col 1
          i === 0 ? grp.voucherNumber : '',   // col 2
          vendorCell,                          // col 3
          i === 0 ? grp.narration     : '',   // col 4
          r.accountCode ?? '',                 // col 5
          descLines,                           // col 6
          r.dbCrFlag    ?? '',                 // col 7
          this.fmt(amt),                       // col 8 — single Amount column
  ...
  downloadExcel(): void {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Debit Note Register');
    XLSX.writeFile(wb, `DebitNoteRegister_${this.accPeriodLabel}.xlsx`);
  }
