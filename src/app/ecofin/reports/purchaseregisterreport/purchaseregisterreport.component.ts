...
import { PurchaseRegisterService } from 'src/app/shared/services/reports/purchaseregisterreport.service';
...
@Component({
  selector: 'app-purchaseregisterreport',
  templateUrl: './purchaseregisterreport.component.html',
  styleUrls: ['./purchaseregisterreport.component.css']
})
export class PurchaseregisterreportComponent implements OnInit {

  isLoading = false;
  today = '';
  filterForm!: FormGroup;

  // Accounts populated from API (Purchase detail accounts: DBCRFLAG = 'D' in CFN_PURJDETAIL)
  accounts: any[] = [];
  voucherGroups: PurchaseVoucherGroup[] = [];
  ...
  constructor(
    ...
    private svc: PurchaseRegisterService,
  ...
  // Loads distinct Purchase detail accounts (DBCRFLAG = 'D' in CFN_PURJDETAIL)
  loadAccounts(): void {
    this.isLoading = true;
    this.svc.getPurchaseAccounts()
      .pipe(finalize(() => (this.isLoading = false)))
      .subscribe({
        next: (resp: any) => {
          this.accounts = Array.isArray(resp?.data) ? resp.data : [];
        },
        error: () => this.alertService.error('Failed to load purchase accounts.')
      });
  }
  ...
  onRetrieve(): void {
    ...
    this.svc.getPurchaseReport({
      ...
      .subscribe({
        ...
        error: () => this.alertService.error('Failed to retrieve Purchase Register.')
      });
  }
  ...
  private groupByVoucher(rows: any[]): PurchaseVoucherGroup[] {
    const map = new Map<string, PurchaseVoucherGroup>();
    for (const r of rows) {
  ...
  trackByVoucherGroup = (_: number, g: PurchaseVoucherGroup) => g.voucherNumber;
  ...
  downloadPdf(): void {
    ...
    doc.text('PURCHASE REGISTER', pageW / 2, 13, { align: 'center' });
    doc.setFontSize(10);
    ...
    for (const grp of this.voucherGroups) {
      grp.rows.forEach((r, i) => {
        ...
        body.push([
          ...
          r.tdsCode ? `${r.tdsCode}\n${r.tdsDescription ?? ''}` : '',
          r.tdsAmount ? this.fmt(Number(r.tdsAmount)) : '',
          `${r.detailAccountCode ?? ''}\n${r.detailAccountDesc ?? ''}`,
          isD ? this.fmt(amt) : '',
          !isD ? this.fmt(amt) : ''
        ]);
    ...
    (doc as any).autoTable({
      ...
      head: [['Vchr Date', 'Vchr No', 'Onhold No', 'Hdr A/C', 'Hdr Sub A/C', 'Ref No/Date', 'Bill No/Date', 'Bill Amt', 'TDS Code', 'TDS Amt', 'Detail A/C', 'Debit', 'Credit']],
      body,
      theme: 'grid',
      ...
      columnStyles: {
        0: { cellWidth: 16 }, 1: { cellWidth: 22 }, 2: { cellWidth: 20 },
        3: { cellWidth: 26 }, 4: { cellWidth: 24 }, 5: { cellWidth: 18 },
        6: { cellWidth: 18 }, 7: { cellWidth: 16, halign: 'right' as const },
        8: { cellWidth: 18 }, 9: { cellWidth: 14, halign: 'right' as const },
        10: { cellWidth: 26 },
      ...
      didParseCell: (data: any) => {
        ...
        if (meta.type === 'vtotal') {
          ...
          if (col === 10) data.cell.styles.halign = 'right';
          if (col === 11) { data.cell.styles.halign = 'right'; data.cell.styles.textColor = [255, 0, 38]; }
          if (col === 12) { data.cell.styles.halign = 'right'; data.cell.styles.textColor = [0, 23, 252]; }
        }
        if (meta.type === 'gtotal') {
          data.cell.styles.fillColor = this.G1; data.cell.styles.fontStyle = 'bold';
    ...
    doc.save(`PurchaseRegister_${v.fromDate}_${v.toDate}.pdf`);
  }
  ...
  downloadExcel(): void {
    ...
    aoaRows.push([`PURCHASE REGISTER — ${v.fromDate} TO ${v.toDate}`, ...Array(12).fill('')]); rowTypes.push('title');
    aoaRows.push(Array(13).fill('')); rowTypes.push('blank');
    aoaRows.push(['Vchr Date', 'Vchr No', 'Onhold No', 'Hdr A/C', 'Hdr A/C Desc', 'Hdr Sub A/C', 'Ref No/Date', 'Bill No', 'Bill Amt', 'TDS Code', 'TDS Amt', 'Detail A/C', 'Debit', 'Credit']); rowTypes.push('header');
    ...
    for (const grp of this.voucherGroups) {
      ...
      aoaRows.push([...Array(12).fill(''), grp.debitTotal, grp.creditTotal]); rowTypes.push('vtotal');
    }
    ...
    ws['!cols'] = [{ wch: 12 }, { wch: 16 }, { wch: 16 }, { wch: 10 }, { wch: 26 }, { wch: 22 }, { wch: 16 }, { wch: 14 }, { wch: 14 }, { wch: 10 }, { wch: 14 }, { wch: 28 }, { wch: 16 }, { wch: 16 }];
    ws['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 13 } }, { s: { r: 1, c: 0 }, e: { r: 1, c: 13 } }];
    this.applyExcelStyles(ws, aoaRows, rowTypes, [8, 10, 12, 13], 14);
    XLSX.utils.book_append_sheet(wb, ws, 'Purchase Register');
    XLSX.writeFile(wb, `PurchaseRegister_${v.fromDate}_${v.toDate}.xlsx`);
  }
